...
let carModels = [];
for (let i = 0; i < inventory.length; i++) {
    let currentcarModel=inventory[i].car_model;
...
let carYears = [];
for(let i=0; i<inventory.length;i++){
     let yearcarYears=inventory[i].car_year;
     carYears.push(yearcarYears);
}
...
let BMWAndAudi = [];
for(let i=0; i<inventory.length;i++){
    let currentMake=inventory[i].car_make;
    if(currentMake ==='BMW'|| currentMake ==='Audi'){
